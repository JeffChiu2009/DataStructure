...
class NodeTest(unittest.TestCase):
    ...
    def test_init(self):
        # self is an istance of this class
        data = 'ABC'
        node = Node(data)
...
class LinkedListTest(unittest.TestCase):
    ...
    def test_insert(self):
        ll = LinkedList()
        ll.insert(0, 'B')  # append('B')
        assert ll.head.data == 'B'  # new head (at index 0)
        assert ll.tail.data == 'B'  # new tail (at index 0)
        assert ll.size == 1
        ll.insert(0, 'A')  # prepend('A')
        assert ll.head.data == 'A'  # new head (at index 0)
        assert ll.tail.data == 'B'  # unchanged (now at index 1)
        assert ll.size == 2
        ll.insert(2, 'D')  # append('D')
        assert ll.head.data == 'A'  # unchanged (at index 0)
        assert ll.tail.data == 'D'  # new tail (now at index 2)
        assert ll.size == 3
        ll.insert(2, 'C')  # insert 'C' between 'B' and 'D'
        assert ll.head.data == 'A'  # unchanged (at index 0)
        assert ll.tail.data == 'D'  # unchanged (now at index 3)
        assert ll.size == 4
        with self.assertRaises(ValueError):
            ll.insert(5, 'X')  # index too high
        with self.assertRaises(ValueError):
            ll.insert(-1, 'Y')  # index too low
